...
struct SimpleKeySequence_t {
  Qt::KeyboardModifiers modifiers;
  Qt::Key key;
};

auto keyPressEvent = [](QEvent *event) -> optional<QKeyEvent*> {
...
struct AlphabeticInput_t {};
struct NumericInput_t    {};


auto keyPress = [](QEvent * event) -> optional<SimpleKeySequence_t> {
  if (const auto keyEvent = keyPressEvent(event); keyEvent) {
    return SimpleKeySequence_t{keyEvent.value()->modifiers(), static_cast<Qt::Key>(keyEvent.value()->key())};
  }
  else
    return {};
};

bool match(Qt::KeyboardModifiers modifiers, const std::vector<Qt::KeyboardModifier>& with) {
  assert(size(with) > 0);
  const auto target_modifier = [&with]() {
    QFlags<Qt::KeyboardModifier> acc({Qt::NoModifier});
    for (int i = 0; i < size(with); ++i) {
      acc |= QFlags<Qt::KeyboardModifier>({with[0]});
    }
    return acc;
  }();
  return (QFlags<Qt::KeyboardModifier>::Int(modifiers) ^ QFlags<Qt::KeyboardModifier>::Int(target_modifier)) == 0;
}

bool match(Qt::Key key, Qt::Key with) {
  return key == with;
}

bool match(SimpleKeySequence_t keySequence, const std::vector<Qt::KeyboardModifier>& mod, Qt::Key key) {
  return match(keySequence.modifiers, mod) and match(keySequence.key, key);
};

}

bool QLineEditEmacsStyleFilter::eventFilter(QObject *obj, QEvent *event) {
  if (const auto infoKeyPress = keyPress(event); infoKeyPress) {
    const auto instanceLineEdit = qobject_cast<QLineEdit*>(obj);
    if (match(infoKeyPress.value(), {Qt::ControlModifier}, Qt::Key_A)) {
        instanceLineEdit->setCursorPosition(0);
        return true;
    }
    return false;
  }
...
bool QTreeViewEmacsStyleFilter::eventFilter(QObject *obj, QEvent *e) {
  if (const auto infoKeyPress = keyPress(e); infoKeyPress) {
    const auto instanceTreeView = qobject_cast<QTreeView*>(obj); 
    if (match(*infoKeyPress, {Qt::NoModifier}, Qt::Key_Tab)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Right, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ShiftModifier}, Qt::Key_Tab)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Left, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_P)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Up, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_N)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Down, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_E)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Right, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_A)) {
      QKeyEvent event(QEvent::KeyPress, Qt::Key_Left, Qt::NoModifier);
      QApplication::sendEvent(instanceTreeView, &event);
      return true;
    }
    ...
    return parent->eventFilter(parent, e);
  }
}
 

bool TocWidget::eventFilter(QObject* o, QEvent* e) {
  if (const auto infoKeyPress = keyPress(e); infoKeyPress) {
    const auto instanceTocWidget = qobject_cast<TocWidget*>(o);
    if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_S)) {
      filterLE->setFocus();
      this->setFocusProxy(filterLE);
      qDebug("TocWidget Filter Tab Key Pressed");
      return true;
    }
    else if (match(*infoKeyPress, {Qt::ControlModifier}, Qt::Key_N)) {
      tocTV->setFocus();
    }
    else if (match(*infoKeyPress, {Qt::NoModifier}, Qt::Key_Return)) {
      goTo(tocTV->currentIndex());
      gui_view_.setFocus();
...
{
  setupUi(this);
  emacsLE = new QLineEditEmacsStyleFilter(this);
  emacsTV = new QTreeViewEmacsStyleFilter(this);
  ...
  filterLE->installEventFilter(emacsLE);
  tocTV->installEventFilter(emacsTV);
